...
export const fetchCart = createAsyncThunk('cart/fetchCart', async (_, { getState }) => {
  ...
  if (customer) {
    ...
    const totalPrice = items.reduce((sum, item) => sum + (item.selectedVariant?.price || 0) * item.quantity, 0)
    return { items, totalPrice }
  }
})

export const addItemToCart = createAsyncThunk('cart/addItem', async (item, { getState }) => {
  const { customer } = getState().auth
  if (customer) {
...
export const removeItemFromCart = createAsyncThunk('cart/removeItem', async (itemIndex, { getState }) => {
  const { customer } = getState().auth
  if (customer) {
    const res = await api.delete(ENDPOINTS.REMOVE_FROM_CART(itemIndex))
    return res.data
  } else {
    const items = loadFromLocalStorage()
